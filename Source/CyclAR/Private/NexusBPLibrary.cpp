...
bool UNexus::StreamDimension(FString& stream) {
	...
	UE_LOG(LogTemp, Warning, TEXT("%s nl"), *stream);
	// split stream into ports 
	/*string ProcessStr = fData;
	istringstream ss(fData);
	string token;
	TArray<string> ParsedData; 
	
	while (std::getline(ss, token, ';')) {
		ParsedData.Add(token);
	}

	for (string DataChunk : ParsedData)
	{
		UE_LOG(LogTemp, Warning, TEXT("%hs"), UTF8_TO_TCHAR(DataChunk.c_str()));
	}*/
	return true;
}
...
// orientation
void UNexus::Orientation(const FString stream, FVector& EulerRot, FRotator& orientation) {
	...
	if (_data.length() > 9) {
		...
		if (_xAxis.length() > 0) {
			EulerRot.X = std::stof(_xAxis);
			orientation.Yaw = EulerRot.X;
		}
		...
		if (_yAxis.length() > 0) {
			EulerRot.Y = std::stof(_yAxis);
			orientation.Roll = EulerRot.Y;
		}
		...
		if (_zAxis.length() > 0) {
			EulerRot.Z = std::stof(_zAxis);
			orientation.Pitch = EulerRot.Z;
		}
	}
}
